...
\begin{subquestion}
    ...
    The bound $|\hat{p} - \hat{p}'| = \mathcal{O}(1/T)$ is asymptotic as the gap shrinks as $T \to \infty$, but for any fixed length $T^{*}$ it is merely small, not zero. 
    ...
    The lemma does not say transformers fail at parity for short strings.

    What it does say is that no fixed parameter setting recognizes \texttt{Odd-1} for all lengths simultaneously.
