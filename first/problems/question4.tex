...
\begin{subquestion}
    ...
    Since $s_t$ is $C^{\infty}$ on $\mathbb{R}^{T}$, the multivariate MVT gives a point $\mathbf{c}_T = \lambda \mathbf{a}_T + (1-\lambda)\mathbf{b}_T$ on the segment between $\mathbf{a}_T$ and $\mathbf{b}_T$ such that
    \begin{equation*}
...
\begin{subquestion}
    ...
    Propagating $\|\mathbf{x}_{t'}^{(\ell)} - \mathbf{x}_{t'}^{\prime(\ell)}\| = \mathcal{O}(1/T)$ through the bilinear logit structure $l_{t,t'} = \mathbf{x}_t^{(\ell)\top}\mathbf{W}_K^{(\ell)}\mathbf{W}_Q^{(\ell)\top}\mathbf{x}_{t'}^{(\ell)}$ gives $|l_{t,t'}^{(\ell)} - l_{t,t'}^{\prime(\ell)}| = \mathcal{O}(1/T)$ for $t' \ne t^*$. 
    ...
    \paragraph{Combine}
    Substituting the two bounds into the truncated Difficult Part and using that the sum has exactly $T - 1$ terms, we get Eq.~(53):
    \begin{equation*}
...
\begin{subquestion}
    We prove Eqs.~(55) and (56) jointly by induction on $\ell$, then apply the bound to $\mathbf{h}(\mathbf{y})$.
    ...
    Take Eq.~(54b)'s four terms and bound each:
    \begin{itemize}
        \item \emph{Term 1} ($\sum_{t' \ne t^*} |s - s'|\,\|\mathbf{x}_{t'}^{(\ell)}\|$): the inductive hypothesis says $\|\mathbf{x}_{t'}^{(\ell)} - \mathbf{x}_{t'}^{\prime(\ell)}\| = \mathcal{O}(1/T)$ for $t' \ne t^*$, so $\|\mathbf{w}_{T,d}^{(\ell)}\|_{\infty} = \mathcal{O}(1/T)$ for every $d \in [D]$. Part~(i) directly gives $\mathcal{O}(1/T)$.
    ...
    \end{itemize}
    Summing the four $\mathcal{O}(1/T)$ contributions yields $\Delta_t^{(\ell)} = \mathcal{O}(1/T)$, which is Eq.~(55).
    ...
    \end{equation*}
    Splitting on the value of $t$ and using Eq.~(55) together with the inductive hypothesis,
    \begin{equation*}
        \|\mathbf{x}_t^{(\ell+1)} - \mathbf{x}_t^{\prime(\ell+1)}\|
        \;\le\; L_{\mathbf{f}}\bigl(\mathcal{O}(1/T) + \|\mathbf{x}_t^{(\ell)} - \mathbf{x}_t^{\prime(\ell)}\|\bigr)
        \;=\;
        \begin{cases} \mathcal{O}(1) & \text{if } t = t^*, \\ \mathcal{O}(1/T) & \text{otherwise,} \end{cases}
    \end{equation*}
...
\begin{subquestion}
    We now apply Lemma~1 to the \texttt{Odd-1} acceptor and draw out its consequences.

    \paragraph{(i) Parity of $\mathbf{y}'$}
    Suppose $\mathbf{y} \in \texttt{Odd-1}$, so $\mathbf{y}$ contains $2k+1$ ones for some $k \in \mathbb{N}$. Flipping a single bit at position $t^*$ either turns a $0$ into a $1$ (giving $2k+2$ ones) or a $1$ into a $0$ (giving $2k$ ones). Either way, the count's parity flips from odd to even, so
    \begin{equation*}
        \mathbf{y}' \;\notin\; \texttt{Odd-1}
    \end{equation*}
    ...
    From part~(j), $\|\mathbf{h}(\mathbf{y}) - \mathbf{h}(\mathbf{y}')\| = \mathcal{O}(1/T)$. Since the readout vector $\mathbf{w}$ is fixed (independent of $T$), Cauchy--Schwarz gives
    \begin{equation*}
...
\begin{subquestion}
    ...
    \emph{Case $\epsilon > 0$.} The denominator $\sqrt{\sigma^{2}(\mathbf{x}) + \epsilon} \ge \sqrt{\epsilon} > 0$ is bounded away from zero, so $\operatorname{LN}$ is smooth with a bounded gradient on $\mathbb{R}^{D}$, hence Lipschitz with some constant $L_{\operatorname{LN}}$. 
    The composition $\mathbf{f} \circ \operatorname{LN}$ (or $\operatorname{LN} \circ \mathbf{f}$) is then Lipschitz with constant $L_{\mathbf{f}} \cdot L_{\operatorname{LN}}$. 
    ...
    \emph{Case $\epsilon = 0$.} The denominator $\sigma(\mathbf{x})$ vanishes whenever the entries of $\mathbf{x}$ are equal (constant inputs), and the gradient of $\operatorname{LN}$ blows up near that set. So $\operatorname{LN}$ is \emph{not} globally Lipschitz, and there is no finite $L_{\operatorname{LN}}$ to absorb. 
    The Lipschitz step in part~(j) breaks: a perturbation of size $\mathcal{O}(1/T)$ to the pre-normalization activations can be amplified arbitrarily after $\operatorname{LN}$, so we cannot conclude $\Delta_t^{(\ell+1)} = \mathcal{O}(1/T)$. 
    The $\mathcal{O}(1/T)$ guarantee is lost.

    \paragraph{(ii) Implication for fixed-length classification}
    The bound $|\hat{p} - \hat{p}'| = \mathcal{O}(1/T)$ is asymptotic: the gap shrinks as $T \to \infty$, but for any fixed length $T^{*}$ it is merely \emph{small}, not zero. 
    Concretely, $\|\mathbf{h}(\mathbf{y}) - \mathbf{h}(\mathbf{y}')\| \ge c/T^{*}$ for some constant $c > 0$, so the representations of $\mathbf{y}$ and $\mathbf{y}'$ remain distinct for every $|\mathbf{y}| \le T^{*}$.

    Hence a soft-attention transformer can in principle classify all strings of length up to $T^{*}$ correctly: choose the readout $\mathbf{w}$ with $\|\mathbf{w}\|$ scaling like $T^{*}$ so that the small representation gap is amplified through the sigmoid past the $0.5$ decision boundary. 
    The lemma does \emph{not} say transformers fail at parity for short strings.

    What it \emph{does} say is that no fixed parameter setting recognizes \texttt{Odd-1} for \emph{all} lengths simultaneously: as $|\mathbf{y}|$ grows beyond $T^{*}$, the same readout no longer separates $\mathbf{y}$ from $\mathbf{y}'$, and the classifier eventually agrees on inputs of opposite parity. 
    Recognition of \texttt{Odd-1} is therefore length-uniformly impossible, even though it is achievable on any bounded-length subset.
\end{subquestion}
