...
\begin{subquestion}
    Absorb the value projection $\mathbf{W}_V^{(\ell)}$ into the MLP $\mathbf{f}$ by exhibiting a function $\mathbf{f}'$ s.t.
    \begin{equation*}
...
\begin{subquestion}
    Prove Eqs.~(55) and (56) jointly by induction on $\ell$, then apply the bound to $\mathbf{h}(\mathbf{y})$.
    ...
    \emph{Case $t = t^*$.} 
    The query side $\mathbf{x}_{t^*}^{(\ell)} - \mathbf{x}_{t^*}^{\prime(\ell)}$ is only $\mathcal{O}(1)$ by the inductive hypothesis, so the bilinear logit propagation in part~(i) does \emph{not} apply at the query position and Term~1 cannot be sharpened. 
...
\begin{subquestion}
    ...
    So $\operatorname{LN}$ is \emph{not} globally Lipschitz, and there is no finite $L_{\operatorname{LN}}$ to absorb. 
    ...
    The bound $|\hat{p} - \hat{p}'| = \mathcal{O}(1/T)$ is asymptotic as the gap shrinks as $T \to \infty$, but for any fixed length $T^{*}$ it is merely \emph{small}, not zero. 
    $\|\mathbf{h}(\mathbf{y}) - \mathbf{h}(\mathbf{y}')\| \ge c/T^{*}$ for some constant $c > 0$, so the representations of $\mathbf{y}$ and $\mathbf{y}'$ remain distinct for every $|\mathbf{y}| \le T^{*}$.

    Hence a soft-attention transformer can in principle classify all strings of length up to $T^{*}$ correctly by choosing the readout $\mathbf{w}$ with $\|\mathbf{w}\|$ scaling like $T^{*}$ so that the small representation gap is amplified through the sigmoid past the $0.5$ decision boundary. 
    The lemma does \emph{not} say transformers fail at parity for short strings.

    What it \emph{does} say is that no fixed parameter setting recognizes \texttt{Odd-1} for \emph{all} lengths simultaneously.
    As $|\mathbf{y}|$ grows beyond $T^{*}$, the same readout no longer separates $\mathbf{y}$ from $\mathbf{y}'$, and the classifier eventually agrees on inputs of opposite parity. 
