...
Define a modified PFSA over an expanded state space $\states'$ where each original state is duplicated, meaning $|\states'| = 2|\states|$. 
This allows the transition matrix to accumulate the path length $k$ as it is raised to higher powers.

Define a new transition block matrix $\mathbf{M} \in \R^{2|\states| \times 2|\states|}$, a new initial state vector $\initf' \in \R^{2|\states|}$, and a new stopping vector $\finalf' \in \R^{2|\states|}$ as follows:
\begin{equation}
...
\begin{align}
    ...
    &= \begin{pmatrix} \initf^\top \transMtx^k & \initf^\top (k\,\transMtx^k) \end{pmatrix}
       \begin{pmatrix} \mathbf{0} \\ \finalf \end{pmatrix} = \initf^\top k\,\transMtx^k \finalf
\end{align}
...
\begin{itemize}
    ...
    \item The \texttt{allsum} algorithm evaluates the infinite sum by calculating the inverse $(\mathbf{I} - \mathbf{M})^{-1}$, or by utilizing Gaussian elimination to solve the equivalent linear system. Matrix inversion and solving linear systems both have a cubic time complexity relative to the size of the matrix.
    ...
    \item Because constants can be dropped in Big-O notation, $\bigo(8|\states|^3)$ simplifies to $\bigo(|\states|^3)$.
\end{itemize}
...
To elaborate further on the properties of finite state language models, we can also bypass the block matrix construction to yield a slightly more memory efficient algorithm that still meets the $\bigo(|\states|^3)$ requirement. 
We know from geometric series properties of matrices that $\sum_{k=0}^{\infty} \transMtx^k = (\mathbf{I} - \transMtx)^{-1}$. 
If we evaluate the infinite sum $\sum_{k=0}^{\infty} k\, \transMtx^k$ by factoring out $\transMtx$ and rewriting it as a matrix derivative, we arrive at:
\begin{equation}
    \sum_{k=0}^{\infty} k\, \transMtx^k = \transMtx (\mathbf{I} - \transMtx)^{-2} = (\mathbf{I} - \transMtx)^{-1} \transMtx (\mathbf{I} - \transMtx)^{-1}
\end{equation}

This means the mean string length can also be written algebraically as $\mu = \initf^\top (\mathbf{I} - \transMtx)^{-1} \transMtx (\mathbf{I} - \transMtx)^{-1} \finalf$. 
We can compute this using a sequence of linear algebraic operations:
\begin{enumerate}
...
The sum of these sequential execution times is $\bigo(|\states|^3) + \bigo(|\states|^2) + \bigo(|\states|^3) + \bigo(|\states|)$, which perfectly asymptotes to the required $\bigo(|\states|^3)$ runtime. Both methods are correct, though the block matrix approach fulfills the hint to deploy the existing \texttt{allsum} operation as a black box.
...
\begin{enumerate}
    ...
    \item \textbf{Solve First System}: $(\mathbf{I} - \transMtx) \mathbf{v}_1 = \mathbf{v}_0$ for $\mathbf{v}_1$. This computes $(\mathbf{I} - \transMtx)^{-1}\finalf$.
    \item \textbf{Solve Second System}: $(\mathbf{I} - \transMtx) \mathbf{v}_2 = \mathbf{v}_1$ for $\mathbf{v}_2$. This computes $(\mathbf{I} - \transMtx)^{-2}\finalf$.
    \item \textbf{Solve Third System}: $(\mathbf{I} - \transMtx) \mathbf{v}_3 = \mathbf{v}_2$ for $\mathbf{v}_3$. This computes $(\mathbf{I} - \transMtx)^{-3}\finalf$.
...
\end{equation}
Group this sum by the destination state $\stateq'$. 
By definition, the transition matrix entry $\transMtx_{\stateq, \stateq'}$ is the sum of the weights $w$ of all transitions from $\stateq$ to $\stateq'$. 
Therefore:
\begin{equation}
...
To solve, let $\mathbf{H} \in \R^{|\states|}$ be a column vector where the $\stateq$-th entry is $\ent(\Pi_\stateq)$. 
Represent the system of equations for all states as a single matrix equation:
\begin{equation}
...
The overall entropy of the language model is the entropy of the path distribution. 
Since $\automaton$ is deterministic, distinct paths produce distinct strings, so $\ent(\pLM) = \ent(\Pi)$. 
Decompose a path into its starting state $\stateq_0$ and the remainder by the chain rule of entropy,
\begin{equation}
    \ent(\Pi) = \ent(\stateq_0) + \ent(\Pi \mid \stateq_0) = \ent(\initf) + \expectation_{\stateq_0 \sim \initf}\!\big[\ent(\Pi_{\stateq_0})\big] = \ent(\initf) + \initf^\top \mathbf{H}
\end{equation}
where $\ent(\initf) = -\sum_{\stateq \in \states} \initf_\stateq \log \initf_\stateq$ is the entropy of the initial state distribution.

Substitute the solved vector $\mathbf{H}$ into the final equation:
\begin{equation}
    \ent(\pLM) = \ent(\initf) + \initf^\top (\mathbf{I} - \transMtx)^{-1}\,\boldsymbol{\xi}
\end{equation}
In the special case where $\automaton$ has a unique initial state, $\initf$ is a point mass and $\ent(\initf) = 0$, recovering $\ent(\pLM) = \initf^\top (\mathbf{I} - \transMtx)^{-1}\,\boldsymbol{\xi}$.
\end{subquestion}
